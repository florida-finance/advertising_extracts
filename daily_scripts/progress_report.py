...
print("Pulling Current Year Digital Numbers")
...
print("Pulling Current Year Total Numbers")
...
print("Pulling Prior 2 Years Digital Numbers")
...
print("Pulling Prior 2 Years Total Numbers")
...
connString = "PROVIDER=MSOLAP;Data Source={0};Database={1}".format('fcwPsqlanl03','Crediting')
query = 'SELECT NON EMPTY {{ [Measures].[Net]}} ON COLUMNS, NON EMPTY {{ ([Credit Sales Assignment].[Sales Assignment Sub Team ID].[Sales Assignment Sub Team ID].ALLMEMBERS * [Credit Sales Assignment].[Sales Assignment Code].[Sales Assignment Code].ALLMEMBERS ) }} DIMENSION PROPERTIES MEMBER_CAPTION, MEMBER_UNIQUE_NAME ON ROWS FROM ( SELECT ( {{ [Reporting Date].[Fiscal Year].&[{0}] }} ) ON COLUMNS FROM ( SELECT ( {{ [Company].[Company Code].&[OSC], [Company].[Company Code].&[SSC] }} ) ON COLUMNS FROM [Crediting])) WHERE ( [Company].[Company Code].CurrentMember, [Reporting Date].[Fiscal Year].&[{1}] ) CELL PROPERTIES VALUE, BACK_COLOR, FORE_COLOR, FORMATTED_VALUE, FORMAT_STRING, FONT_NAME, FONT_SIZE, FONT_FLAGS'.format(cy,cy)
a = ado(query, connString)
df = pd.DataFrame(data= list(a)).transpose()
df.rename(columns = {0:'sub_team', 2:'sa_id'}, inplace=True)
df = df[['sa_id', 'sub_team']]
sub_team_assignments = df.copy()
...
query = "DELETE FROM sub_team_assignments"
c.execute(sa_text(query).execution_options(autocommit=True))
...
sub_team_assignments.to_sql('sub_team_assignments', c, if_exists='append', index=False)
